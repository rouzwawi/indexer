...
use bitmap_indexer::{BitmapIndex, csv::ingest_csv};
...
/// Test data generation utilities for creating synthetic CSV files
mod test_data_generation {
    ...
    pub fn generate_csv(path: &Path, config: &CsvConfig) -> Result<(), Box<dyn std::error::Error>> {
    ...
    pub fn generate_edge_case_csv(path: &Path) -> Result<(), Box<dyn std::error::Error>> {
        let mut file = File::create(path)?;

        writeln!(file, "name,description,category")?;
        writeln!(file, "simple,Basic value,cat1")?;
        writeln!(file, "\"quoted,value\",\"Value with, comma\",cat2")?;
        writeln!(file, "unicode_αβγ,Value with unicode: αβγδε,cat3")?;
        writeln!(file, "newline_test,\"Value with\nnewline\",cat1")?;
        writeln!(file, "\"\"\"escaped\"\"\",Triple quoted value,cat2")?;
        writeln!(file, "empty_field,,cat3")?;
        writeln!(file, "special!@#$%,Special chars: !@#$%^&*(),cat1")?;

        Ok(())
    }

    /// Create a single-column CSV for edge case testing
    pub fn generate_single_column_csv(path: &Path, num_rows: usize) -> Result<(), Box<dyn std::error::Error>> {
        let mut file = File::create(path)?;

        writeln!(file, "single_col")?;
        for i in 0..num_rows {
            writeln!(file, "value{}", i % 5)?; // 5 unique values cycling
        }
